...
batch_gen = BatchGenerator(TESTING, neg_sample_size=25000)

num_users, num_items, u_nodes, v_nodes, \
    ratings, u_features, v_features = load_data(DATASET, seed=DATASEED,
                                                verbose=False)

graph_info = (num_users, num_items, u_nodes, v_nodes, \
              ratings, u_features, v_features)

u_features, v_features, test_labels, test_u_indices, \
    test_v_indices, class_values = batch_gen(graph_info)


num_users = batch_gen.num_users
num_items = batch_gen.num_items
...
for epoch in range(NB_EPOCH):
    ...
    #         tp += 1
    #     elif outputs[i] == 1 and labels[i] != 1:
    #         fp += 1
